...
/// NUMS point for Taproot: "Nothing Up My Sleeve" point
...
/// Deserialize a compact size integer from a ByteData
...
/// Deserialize a string from a ByteData
...
/// Deserialize a vector of strings from a ByteData
...
/// Check if script is a Pay-to-Public-Key-Hash script
...
/// Extract public key from a transaction input
ECPublicKey? getPubKeyFromInput(VinInfo vin) {
  ...
  if (isP2PKH(spk)) {
    ...
    // Scan for compressed pubkey at the end of scriptSig
...
/// Create a tagged hash for Schnorr signatures
...
/// Produces a Schnorr signature per BIP-340
Uint8List signSchnorr(
  Uint8List msg, // 32-byte message
  Uint8List privKeyBytes, // 32-byte scalar
  Uint8List auxRand, // 32-byte aux
) {
  assert(msg.length == 32, "Message must be 32 bytes");
  assert(auxRand.length == 32, "Auxiliary data must be 32 bytes");
